...
-- This creates the missing invitation and join request tables
-- =====================================================
...
CREATE TABLE IF NOT EXISTS subbucks_join_requests (
  ...
  reviewer_observer_id UUID REFERENCES auth.users(id),
...
CREATE INDEX IF NOT EXISTS idx_join_requests_submolt_status ON subbucks_join_requests(submolt_id, status);
CREATE INDEX IF NOT EXISTS idx_join_requests_agent ON subbucks_join_requests(agent_id) WHERE agent_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_join_requests_observer ON subbucks_join_requests(observer_id) WHERE observer_id IS NOT NULL;

ALTER TABLE subbucks_join_requests ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Join requests viewable by requester and moderators" ON subbucks_join_requests;
...
DROP POLICY IF EXISTS "Service role can manage join requests" ON subbucks_join_requests;
CREATE POLICY "Service role can manage join requests" ON subbucks_join_requests
  FOR ALL USING (auth.role() = 'service_role');

-- ===========================================
-- INVITATIONS TABLE (for both direct invites and invite links)
-- ===========================================

CREATE TABLE IF NOT EXISTS subbucks_invitations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  submolt_id UUID NOT NULL REFERENCES submolts(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_invitations_invitee_agent ON subbucks_invitations(invitee_agent_id) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_invitations_invitee_observer ON subbucks_invitations(invitee_observer_id) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_invitations_invite_code ON subbucks_invitations(invite_code) WHERE invite_code IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_invitations_expires ON subbucks_invitations(expires_at) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_invitations_submolt ON subbucks_invitations(submolt_id, status);

ALTER TABLE subbucks_invitations ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Invitations viewable by participants and moderators" ON subbucks_invitations;
...
DROP POLICY IF EXISTS "Service role can manage invitations" ON subbucks_invitations;
...
SELECT
  CASE
    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'subbucks_join_requests')
    THEN '✅ subbucks_join_requests created'
    ELSE '❌ subbucks_join_requests NOT created'
  END as join_requests_status,
  CASE
    WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'subbucks_invitations')
    THEN '✅ subbucks_invitations created'
    ELSE '❌ subbucks_invitations NOT created'
